...
class ArcballControl {
  ...
  constructor(canvas, updateCallback) {
    ...
    canvas.addEventListener('pointerup', () => {
      ...
      // Force immediate snapping when releasing
      if (this.snapTargetDirection) {
        this.snapDirection = vec3.clone(this.snapTargetDirection);
      }
    });
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (this.snapTargetDirection) {
        // Increased snapping intensity for more responsive selection
        const SNAPPING_INTENSITY = 0.4;
        const a = this.snapTargetDirection;
        const b = this.snapDirection;
        const sqrDist = vec3.squaredDistance(a, b);
        
        // Enhanced distance factor for faster snapping at higher distances
        const distanceFactor = Math.max(0.2, 1 - sqrDist * 8);
        ...
        // Debug info
        if (sqrDist > 0.01) {
          console.log("Snapping distance:", Math.sqrt(sqrDist).toFixed(3), "Factor:", distanceFactor.toFixed(3));
        }
      }
    }
...
class InfiniteGridMenu {
  ...
  #onControlUpdate(deltaTime) {
    ...
    if ((!isMoving && this.control.isPointerDown === false) || 
        this.nearestVertexIndex !== nearestVertexIndex) {
      
      this.nearestVertexIndex = nearestVertexIndex;
      
      const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
      console.log("InfiniteGridMenu: Nearest vertex =", nearestVertexIndex, "Item index =", itemIndex);
...
export default function InfiniteMenu({ items = [], onSelect }) {
  ...
  // Keep track of previous index to avoid redundant calls
  const prevIndexRef = useRef(-1);
  // Store the sketch reference to avoid recreating it
  const sketchRef = useRef(null);
  // Store the handler to avoid recreation
  const handlerRef = useRef(null);

  // Create the handler only once - without dependencies that trigger recreation
  useEffect(() => {
    // Create a handler that closes over the current state values
    handlerRef.current = (index) => {
      // Only update if the index changed to avoid loops
      if (index !== prevIndexRef.current) {
        console.log("Selected item changed from", prevIndexRef.current, "to", index);
        prevIndexRef.current = index;
        
        // Use current items from props
        ...
        // Call onSelect if provided, but don't create dependency
        if (onSelect && item) {
          console.log("Calling onSelect with:", item.id || item.title);
          // Access onSelect via closure rather than dependency
  ...
  useEffect(() => {
    ...
    // If items change, need to update the grid
    if (canvas) {
      if (!sketchRef.current) {
        console.log("Creating new InfiniteGridMenu");
        sketchRef.current = new InfiniteGridMenu(
          canvas,
          items.length ? items : defaultItems,
          // Use the handler from ref
          (index) => handlerRef.current && handlerRef.current(index),
          setIsMoving,
          (sk) => sk.run()
        );
      } else if (sketchRef.current.items !== items) {
        // Only update items if they've actually changed
        console.log("Updating grid with new items");
        // Update items and recreate texture
        sketchRef.current.updateItems(items.length ? items : defaultItems);
      }
    }
    ...
    return () => {
      ...
      if (sketchRef.current && sketchRef.current.gl) {
        // Don't destroy on item updates
      ...
      {activeItem && (
        <>
          ...
          {/* <div onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`}>
            <p href={activeItem.link} className="action-button-icon">&#x2197;</p>
          </div> */}
        </>
      )}
    </div>
