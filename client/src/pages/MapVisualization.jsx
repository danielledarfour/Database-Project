      ...
      <div className="container-custom mx-auto px-4 md:px-8">
        ...
        >
          ...
          <p className="text-gray-300 max-w-3xl mb-4">
            Select a data type and timeframe, then click on any state to view detailed information.
          </p>
        </motion.div>
        ...
        <div className="bg-eerie-black rounded-lg shadow-md border border-gray-800 p-3 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <label htmlFor="data-type" className="block text-sm font-medium text-gray-300 mb-1">
                Data Type
              </label>
              <select
                id="data-type"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <h3 className="text-md font-medium text-mint mb-3 pb-1 border-b border-gray-800">
                  {dataType === 'crime' ? 'Crime Statistics' : 'Employment Statistics'}
                </h3>
                    ...
                    <>
                      <div>
                        ...
                        <div className="flex items-center">
                          ...
                          <span className="ml-2 text-sm text-gray-300">45%</span>
                        </div>
                      </div>
                      <div>
                        <h4 className="text-sm font-medium text-gray-300 mb-1">
                    ...
                    <div className="bg-eerie-black p-2 rounded-md flex justify-between border border-gray-800">
                      {[2019, 2020, 2021, 2022, 2023].map((year) => (
                        <div key={year} className="text-center">
                          <div 
                            className={`mx-auto w-2 ${dataType === 'crime' ? 'bg-red-400' : 'bg-green-400'} rounded-t-sm`} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-eerie-black rounded-lg p-4 border border-gray-800">
              <h3 className="text-md font-medium text-mint mb-2 pb-1 border-b border-gray-800">
                {dataType === 'crime' ? 'Highest Crime Rates' : 'Highest Employment'}
              </h3>
              <ul className="space-y-2">
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">New Mexico</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '82.3%' : '75.2%'}</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">Louisiana</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '79.8%' : '71.5%'}</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">Nevada</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '78.1%' : '69.7%'}</span>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-eerie-black rounded-lg p-4 border border-gray-800">
              <h3 className="text-md font-medium text-mint mb-2 pb-1 border-b border-gray-800">
                {dataType === 'crime' ? 'Lowest Crime Rates' : 'Lowest Employment'}
              </h3>
              <ul className="space-y-2">
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">Maine</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '15.2%' : '52.3%'}</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">New Hampshire</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '17.8%' : '54.6%'}</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-gray-300">Vermont</span>
                  <span className="font-medium text-white">{dataType === 'crime' ? '19.3%' : '55.1%'}</span>
                </li>
              </ul>
            </div>
                ...
                <div className="mb-2">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm font-medium text-white">
                      {dataType === 'crime' ? '42.7%' : '61.3%'}
                    </span>
                  </div>
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${dataType === 'crime' ? 'bg-red-500' : 'bg-green-500'}`}
                      style={{ width: dataType === 'crime' ? '42.7%' : '61.3%' }}
                    ></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-gray-300">Year-over-Year Change</span>
                    <span className="text-sm font-medium text-white">
                      {dataType === 'crime' ? '-1.2%' : '+2.4%'}
                    </span>
                  </div>
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${dataType === 'crime' ? 'bg-green-500' : 'bg-green-500'}`}
                      style={{ width: dataType === 'crime' ? '35%' : '65%' }}
                    ></div>
                  </div>
                </div>
              </div>
